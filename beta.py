"""
Collective attacker has two steps:
   first, critical region is identified,
   second, the outlier nodes will be added back with re-insertion
...
OUTPUT_DIR = "./output/parameter_tuning/beta/"
...
def add_back(G, SG, n, cut_nodes, deleted_nodes = None, beta = None, limit = None):
...
def arg_setup():
    ...
    parser.set_defaults(reinsertion = True)
...
def main(): 
    ...
    with open(fname, 'w') as f:
        ...
        f.write('# Parameters: ' + 'L = ' + str(args.limit) + ', beta = ' + str(args.beta) +
                ', gamma = ' + str(args.log) + ', nruns = ' + str(args.number) +
        ...
        f.write('# run No., objective function value, duration\n')
        for res in out:
            f.write(str(res[0]) + ','  + str(min(res[1], res[2])) + ',' + str(res[3]) + '\n')
