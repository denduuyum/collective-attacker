...
mkdir -p output
rm -v output/parameter_tuning/gamma/gamma_result_summary_04_30_3_noParallel.out
for i in "${!numbers[@]}"; do
    echo -n ${names[i]}
    python3 $1 -l 3 -L ${numbers[i]} -n 30 -b 0.4 ../data/${names[i]} >> output/parameter_tuning/gamma/gamma_result_summary_04_30_3_noParallel.out
    echo
done
